defmodule Tanx.Core.ArenaObjects do
  ...
  """
  def create_missile(arena_objects, x, y, heading, tank_radius, wall_bounce) do
    GenServer.call(arena_objects, {:create_missile, x, y, heading, tank_radius, wall_bounce})
  end
  ...
  def handle_call({:create_missile, x, y, heading, tank_radius, wall_bounce}, {player, _}, state) do
    {:ok, missile}  = Tanx.Core.Missile.start_link(player,
                                                   ...
                                                   tank_radius,
                                                   wall_bounce)
    {:reply, missile, %State{state | objects: state.objects |> Map.put(missile, player)}}
  end
  ...
  def handle_call(:get_objects, {from, _}, state) do
    ...
    {:reply, state.objects |> Map.keys(), %State{state | updater: from, entry_points: entry_points}}
  end
    ...
    entry_points = availability
      |> Enum.reduce(state.entry_points, fn {name, available}, eps ->
        ...
        if ep == nil do
          ...
          if available do
            if ep.avail != :used do
              eps |> Map.put(name, %EntryPointInfo{ep | avail: :clear})
            else
              eps
            end
          else
            eps |> Map.put(name, %EntryPointInfo{ep | avail: :occupied})
          end
        end
      end)
  ...
  defp interpret_entry_point(params, state) do
    ...
    entry_point = state.entry_points |> Map.get(entry_point_name, nil)
    if entry_point == nil do
      ...
      if entry_point.avail == :clear do
        ...
        entry_points = state.entry_points |> Map.put(entry_point_name, entry_point)
        state = %State{state | entry_points: entry_points}
        {params, state}
      else
        {nil, state}
