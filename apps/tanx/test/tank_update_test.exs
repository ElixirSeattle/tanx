defmodule Tanx.TankUpdateTest do
  ...
  setup do
    ...
    {:ok, player} = game |> Tanx.Game.connect(name: "daniel")
  ...
  test "tank tread decrements as it moves backward", %{game: game, player: player} do
    :ok = player |> Tanx.Player.new_tank()
    :ok = player |> Tanx.Player.control_tank(:backward, true)

    game |> Tanx.Game.manual_clock_tick(3000)
    _check_tank(%{player: player, x: -2.0, y: 0.0, heading: 0.0, tread: 0.0})
    game |> Tanx.Game.manual_clock_tick(4000)
    _check_tank(%{player: player, x: -3.0, y: 0.0, heading: 0.0, tread: 0.50})
    game |> Tanx.Game.manual_clock_tick(5000)
    _check_tank(%{player: player, x: -4.0, y: 0.0, heading: 0.0, tread: 0.0})
    game |> Tanx.Game.manual_clock_tick(7000)
  ...
  test "tank stops at arena edge", %{game: game, player: player} do
    :ok = player |> Tanx.Player.new_tank()
    :ok = player |> Tanx.Player.control_tank(:forward, true)
    game |> Tanx.Game.manual_clock_tick(5500)
    _check_tank(%{player: player, x: 9.0, y: 0.0, heading: 0.0})
    game |> Tanx.Game.manual_clock_tick(6000)
    _check_tank(%{player: player, x: 9.5, y: 0.0, heading: 0.0})
    game |> Tanx.Game.manual_clock_tick(6500)
    _check_tank(%{player: player, x: 9.5, y: 0.0, heading: 0.0})
  end
  ...
  test "tank stops at wall", %{game: game, player: player} do
    pi = :math.pi();
    :ok = player |> Tanx.Player.new_tank(heading: pi)
    :ok = player |> Tanx.Player.control_tank(:forward, true)
    game |> Tanx.Game.manual_clock_tick(3000)
    _check_tank(%{player: player, x: -4.0, y: 0.0, heading: pi})
    game |> Tanx.Game.manual_clock_tick(3250)
    _check_tank(%{player: player, x: -4.4, y: 0.0, heading: pi})
    game |> Tanx.Game.manual_clock_tick(3300)
    _check_tank(%{player: player, x: -4.4, y: 0.0, heading: pi})
  end
  ...
  defp _check_tank(%{player: player, x: x, y: y, heading: heading, tread: tread}) do
    ...
    got = view.tanks |> hd()
    want = %Tanx.View.Tank{is_me: true, name: "daniel", x: x, y: y, heading: heading, tread: tread}
  ...
  defp _check_tank(%{player: player, x: x, y: y, heading: heading}) do
    view = player |> Tanx.Player.view_arena_objects()
    got = view.tanks |> hd()
    want = %Tanx.View.Tank{is_me: true, name: "daniel", x: x, y: y, heading: heading}
    assert_in_delta(got.x, want.x, @epsilon)
    assert_in_delta(got.y, want.y, @epsilon)
  end
